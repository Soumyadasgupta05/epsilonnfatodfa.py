 def alpahabet(self):     #function for storing the alphabets
        n1 = int(input("Provide the number of character in the alphabet : "))

        print("input the state names:")
        print("input the alphabets")
        for l in range(0, n1):

            if l==0:
                m = input("enter epsilon as  : ")
                self.A.append(m)
            else:
                m = input("Give the " + str(l + 1) + " th alphabet : ")
                self.A.append(m)
                if l > 0:
                    self.A1.append(m)
        print(" list of all the symbols except the epsilon are : ")
        print(self.A1) #displaying the alphabet list without epsilon
    ...
    def accepting_state_NFA(self):   #accepting the final states
        n2=int(input("Number of accepting states in your NFA "))
        for l1 in range(0,n2):
            self.F.append(input("the "+str(l1+1)+" accepting state of the NFA : ")) #storing the accepting states in the list 'F'
    ...
    def matrix_store(self, i, j):                                   #method for defining the delta function of the NFA
        n=int(input("the number of states at which "+ str(self.Q[i])+ " can transit after getting the symbol " +str(self.A[j])+" : "))
        list = []
        for t in range(0,n):
            list.append(input("the " +str(t+1)+ " th state : "))  #appending all the states to which Q[i] can transit after getting the symbol A[j]
    ...
    def dfa_state_transition(self):
        L = self.dfa_states_store(0, [self.q0])
        l = len(L[0])
        x = int(input(print("Do you want to see the transition of the states of the constructed DFA, when acted upon by the alphabets? If 'yes : press 1, else : press 0.")))
        if x==1:
            for i in range(0,l):
